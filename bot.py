...
async def addanime(event: bot.SimpleBotEvent) -> str:
    try:
        conn = mysql.connector.connect(
            user="root",  # My database is accessible only for the localhost. That's why I didn't set a very secure login/password combination.
            password="root",
            host="localhost",
            port=3306,
            charset="utf8mb4",
        ...
        if last_episode.isnumeric() != True: # domain and episode validation
        ...
        if ext.domain == 'animego' and last_episode.isnumeric() == True:
            ...
            if page.status_code != 200:
                ...
                cur.execute("INSERT INTO animes (title, last_episode, url, id) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE last_episode = VALUES(last_episode)", (title, last_episode, url, id))
                conn.commit()
                conn.close()
        ...
        while notify:
            ...
            for anime in anime_list:
                ...
                if page.status_code != '200':
                    ...
                    dubs = requests.get('https://animego.org/anime/series?dubbing=2&provider=24&episode=' + episode + '&id=' + value, headers=headers).json()['content']
